...
@WebServlet({ "/", "/UserServlet", "/UserServlet/list", "/register", "/signin", "/details", "/UserServlet/likevd",
		"/UserServlet/move/*", "/UserServlet/myfavorite", "/UserServlet/unlike", "/UserServlet/sendemail",
...
//		request.getRequestDispatcher("/views/home.jsp").forward(request, response);
	...
	protected void unLikeVideo(HttpServletRequest request, HttpServletResponse response) throws Exception {
		String userid = XScope.getSession(request, "username").toString();
		String videoid = request.getParameter("VideoID");
		FavoriteDAO daof = new FavoriteDAO();
...
//		request.getRequestDispatcher("/views/home.jsp").forward(request, response);
	...
	protected void sendemail(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		final String username = "quanphph12911@fpt.edu.vn.com";
		final String password = "Phanquan1708";
		try {
			String videoid = request.getParameter("VideoID");
			VideoDAO daov = new VideoDAO();
			Video video = daov.findByID(videoid);
	...
	protected void EmailForgotPass(HttpServletRequest request, HttpServletResponse response)
		...
		if (user != null) {
			...
			String emailContent = "Dear " + user.getFullname() + ", " + "Your PassWord: " + user.getPasswords() + ".";
			try {
				MimeMessage message = new MimeMessage(session);
				message.setFrom(new InternetAddress(username));
	...
	protected void Registeruser(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (user.getId().equals("") || user.getEmail().equals("") || user.getFullname().equals("")
					|| user.getPasswords().equals("")) {
